...
**Answer** There are 8 columns: OBJECTID, ZIPCODE2, ZIPNAME, EXISTING, ZIPCODE1, Shape_Leng, Shape_Area and geometry. 
...
**Answer** This dataframe has 8 columns and 526 rows. It contains data for all of the zip codes in Maryland, including the neighborhood name and geographic data. 
...
**Answer** We're looking at a grey zipcode-level map of Maryland. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** Now we have the tigris package and library installed. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We have a dataframe with 18 columns and 3,234 rows, with detailed information about all of the counties in Maryland. 
...
**Task** Run the following code. Describe the output in the space below.
**Answer** We've created a dataframe with all of the Maryland county-level geographic info from the Census. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** Now we're looking at a grey county-level map of Maryland. Notably, the bay doesn't exist because counties split the bay and their boundaries extend into the water. But Marylanders will not like this version of the map.  
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** Now the county-level map is superimposed over the map with zip code boundaries. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** The dataframe has 2 columns: one with zipcode and another with the number of notices of foreclosure as of March 2023. There are 547 rows. Not every zip code is from Maryland, indicating that perhaps notices are going to owners at addresses out of state. 
...
**Answer** We joined the zip code dataframe with the foreclosure dataframe. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** Now we have a colored map of Maryland zip codes that displays how many forclosures notices were issued by zip as of March 2023. The lighter blue the zip code, the more forclosures. 
...
**Answer** We've loaded the tidyverse, sf, janitor and tidycensus libraries. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We've read in a dataframe of forclosure notices by county in Maryland. We used the slice function to take the first row of data, and then the pivot_longer function to flip the columns and rows so the dataframe is long instead of wide. We've also used get_acs to create a dataframe with population data for those counties from 2021. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We've added a geometry column to the county population dataframe. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We see the two dataframes that we want to join, but the issue is that the county names need some cleaning first. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We've cleaned the county names so that they can be joined. We changed NAME to county, removed title case, removed ", Maryland", removed leading or trailing spaces, renamed the estimate column as population. We saved the dataframe as just the county, population and geometry.  
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We've joined the county population and forclosure dataframes. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We added a column with the rate of notices per 1,000 residents. Charles County has the highest rate, with 2 notices per 1,000 residents. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We've made a color-coded, county-level map of Maryland with the county names displayed. The eye goes straight to Charles County - it's the lightest, meaning it has the highest foreclosure rate in the state. The labels are hard to read in the darker counties like Montgomery, however. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** We see a colored county-level map of Maryland with the foreclosure rate. But it's not labeled well, you'd have to know what the counties are by sight. Also, the key to help you understand the rate is vague. The rate is between zero and two... two what? 
...
**Answer** Now the hot pink has been replaced with orange, and the key has a bunch of decimal places. This still leaves a bit to be desired in my opinion. I don't know enough about Maryland to really read into these trends. I actually prefer the dark blue map above because the gradation makes sense to me and the counties are better labeled there (although maybe grey or another color text would improve legibility). 

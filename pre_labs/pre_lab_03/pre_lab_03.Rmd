...
**Answer** The parsing issues seems to indicate that R is finding values that don't fit with the data type it believes should be in certain columns. Running problems(payments) provides a list of all of the parsing problems that R found. 
...
**Answer** It ran successfully without parsing issues this time and correctly identified the data type in each column. 
...
**Answer** The "Vendor Name" field has data of the character type. "Amount"'s type is double, meaning it contains numbers. "Date" also contains characters. 
...
**Answer** There are 549 rows in the dataset that have a zip code comprised of fewer than five characters. As we discussed in class, these could be foreign zip codes, or New Jersey zip codes where the leading zero has been dropped. Or they could be mistakes, some kind of faulty data entry. 
...
**Answer** There are no loans for 2009. There are transactions that are neither grants nor loans but "NA." Someone typed "L" for loans at one point. There's a random "Contract" in there somewhere. So overall the data isn't as neat as you would ideally hope for. 
...
**Answer** The smallest grant in the data set was in 2009, when the state granted $60 to Land Preservation Trust, Inc. Here we also can see some 9-digit zip codes, which will have to be fixed if the zip code becomes an important variable later. We also see that the letter case over the grantees is inconsistent, some being in all caps and others in title case. 
...
**Answer** The code has eliminated spaces in the column headers. It has eliminated leading and trailing spaces, and replaced the space between words with an underscore. This has been saved as a new dataset called cleaned_md_grant_loans. 
...
**Answer** First we are going to be saving this code into the new data set, cleaned_md_grants_loans. We're taking the old data set and cleaning the names to remove spaces in headers and repalce with underscores. Then we are renaming the "grantor" column to be called "source." Then we are mutating the data in "Source," "Grantee" and "Description" to be in all caps. 
...
**Answer** We see 50 results, so there could be as many as 25 duplicate entries. 
...
**Answer** The new data frame now has 17,740 rows, meaning that 25 duplicates have been eliminated from the original data set. 
...
**Answer** Here we created a new column called "zip 5" which contains only the first five digits of the data appearing in the "zip_code" column. 
...
**Answer** First I want to note that I chose to string detect because in a quick skim of the Source column I saw that the Maryland Tourism Board was listed alongside other agencies. So I figured to include those apparent co-grants instead of excluding them. 

It looks like the 21202 zip code comes in first, having received $8,646,101 from the Maryland Tourism Board (in conjunction with other agencies at times). That zip code includes a slice of downtown Baltimore that goes down to the harbor, a very touristy area, so that checks out. 

...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer** The id column has characters, the seats column has numbers.  
...
**Answer** Providing the number of classes listed per term as far back as this set goes. It's grouping the data by term, then counting the number of classes listed per term. N is the number of entries so it's pulling from the entire list. It's in chronological order, from oldest to most recent terms. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** Now the code has ordered the terms from the one with the most number of courses available for registration, to the term with the fewest courses offered. It's still displaying the same info, the term and the number of courses, but now it's in descending order instead of chronological order. 
...
Compared to the previous code block, how did the output change and what in the code changed it?

**Answer** We've added a new column, the department the class is listen under. We're back in chronological order, with the last term of 2017, and the departments are arranged in alphabetical order. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** We got rid of the department column and now we have a column of the total seats available for each term, as well as the number of courses listed in the middle. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer** Now we're looking at even more columns: the term, the number of courses listed, the total number of seats per term, the mean (average) number of seats per term, and lastly the median number of seats per term. 

The top six terms are all fall terms, which indicates that out of the year, the fall tends to have the most course listings. 
...
**Answer** We got rid of the term and are now most interested in the department: we have the department name, the number of courses listed per department, the total number of seats per department, the mean (average) number of seats per department, the median number of seats per department, the minimum number of seats per department and the maximum number of seats per department.  
...
1.  What departments have grown over time in terms of the number of courses they offer? Which have shrunk? 

2.  What departments have the smallest class sizes? The largest? 

3.  What department is the best at including the syllabus? Which is the worst? 

...
The summary command gives a snapshot of what's happening in each column. For the character based columns, it gives the number of rows and the class and mode (character). For the double (number) valued columns, it gives minimums, maximums and quartiles. 
...
**Answer** It's important to hold the new result so that we can lock in this new data set and then continue to play with it or refine it. This way we can experiment without messing up the original code it took to get to this set; it also helps us keep our code shorter by not constantly working all the way from umd_courses.  
...
**Answer** In this dataset, a total of 41,011 classes either have Theatre as the department or have more than 15 seats. 
...
**Answer:** Lubridate has changed the data type for expenditure_date from <chr> to <date> and reformatted the date to YYYY-MM-DD as it should be. 
...
**Answer** It looks like any date that begins with the same year and month is being conglomerated into a neat group titled with the first of that month. So there many be multiple rows that start with 2022-06- with varying dates, but they're all summed together and called 2022-0601. 
...
**Answer** First it's creating a new column called total_votes that is adding up the votes for Cox, Moore, Lashar, Wallace and Write-ins that tallies all of the votes cast in each of the counties (rows). Then it's creating another new column called pct_moore that shows the percentage of the vote in that given county (row) by taking the votes cast for Moore alone and dividing it by the total votes. 
...
**Answer** It changed the value in the pct_moore column from a decimal to a percentage. 
...
**Answer** From the perspective of Moore's campaign or even rival campaigns, it's very interesting. Moore's team would want to know where he performed the worst so that the next time he's up for reelection they can concentrate resources there. His rivals may see those places as strongholds they can count on and conversely spend fewer resources there in the next election cycle. 
...
**Answer** According to stringr.tidyverse.org, the command str_to_lower() converts to lower case; the command str_to_title() converts to title case, where only the first letter of each word is capitalized.
...
**Answer** Here we are creating a new dataset called maryland_expenses_with_state. The code is pulling from the existing dataset maryland_expenses. It identifying the following parts of addresses as states: Maryland , California , Washington , Louisiana , Floride , North Carolina , Massachussetts , West Virginia , Virginia . Then is is saying that in the new state column, those states should respectively be abbreviated to MD, CA, WA, LA, FL, NC, MA, WV, VA. All other addresses that don't include one of those states should display N/A in the state column. 
...
**Answer** It makes sense to me. This lists the total campaign contributions coming from each state in descending order. Since this is a race for Maryland's governor, it follows that Maryland would be the top state. I can also imagine that NA comes second because it's comprised of the 41 other states that weren't given a specific abbreviation from the previous code. 
...
1. If I'm only interested in the states that contributions came from for the moment, I would see if I can run select(-address) to remove the address column. It's so big and just taking up space. 

2. How can you further categorize dates? We learned how to group by month, can we group by quarter? 

3. Can we break up names into first name and last name columns? I'm not sure what the application may be here, but I'm curious about this notion of breaking down complex bits of data like names and addresses. 

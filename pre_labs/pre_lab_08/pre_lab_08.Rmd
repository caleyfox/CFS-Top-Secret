...
**Answer** baltco_911_calls has 3,822 rows and 4 columns. 
...
**Answer** This code creates a new column that extracts the month from the call date and label=TRUE gives us the abbreviated name of the month instead of a number 1-12. It also groups calls by month, adds up the number of calls per month, and displays the months in descending order from the most calls to the least. 
...
**Answer** This creates a big and beautiful grey box with nothing in it. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** Now we have a grey bar chart with months running chronologically along the x axis and each bar representing the number of calls that month. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** Now the x axis is reordered, arranging the months by total calls from smallest to largest. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**The x and y axis have swapped, so now the bars are displayed in width instead of height. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**This code removed the light grey background. 
...
**Answer** Now the background is robin's egg blue and the type is larger and black instead of grey. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** Now there are clearer labels for the x and y axis, a header and a caption with the data source.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** We have calls arranged by the day and the number of calls on each day for a year starting February 6, 2022. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** We have a line chart of the 911 calls per day, with the x axis being time. It's hard to detect a pattern because the line is so jagged and condensed. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** Now we have a datapoint per week instead of per day. The grid in the background has gotten more specific. The labels on the x axis are now illegible because they're overlapping one another. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** Now the labels on the x axis have rotated 45 degrees so they're more legible than before, but still crowded. 

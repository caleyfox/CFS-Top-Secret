...
**A1.** Here is the link to my chart: https://www.datawrapper.de/_/EKk4U/. I decided to make a bullet bar chart because it seems to be a good format for comparing two numbers, in this case the greater number of votes for Hogan than Cox. I chose dark blue for Hogan and light blue for Cox so his smaller number pops. In my annotations I called out that Cox lost Hogan voters in every county. 
...
md_gov_compare <- md_gov_county|> inner_join(md_gov_county_18, join_by("County","GEOID"))

md_gov_compare <- md_gov_compare|> 
  rename(write_ins_22 = `Write-ins.x`, write_ins_18 = `Write-ins.y`,)|>
  clean_names()
...
write_csv(md_gov_compare, "data/md_gov_compare.csv")
...
**A2.** Here is my chart: https://www.datawrapper.de/_/P6bhX/. I did not code anything here as I was able to create the percentages in Datawrapper using the "stack percentages" function. I hid the totals column because it was eating up half of the chart and misconstruing the visual. I think that the story here is people prefer to vote in person on election day, and in-person voting has bounced back to pre-pandemic levels. 
...
**A3.** Here is my map: https://www.datawrapper.de/_/ypE9U/. I want to know what's going on in the Germantown area that makes overdose calls there so much more prevalent than other areas. 
...
montgomery_2022_overdoses <- montgomery_2022_overdoses |> 
  group_by(zip)|> 
  mutate("pct_of_calls" = (calls_in_zip/total_calls)*100)

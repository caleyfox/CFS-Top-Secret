...
md_expenses$amount <- parse_number(md_expenses$amount)
...
**A1.** See below, the committee "Jain Ashwani Friends of" spent $62,357.38 on Facebook. Sorry, I could not figure out how to exclude NA payees in the time allotted. I found some online articles about omit() and omit.na() functions and took a few cracks at it. I just couldn't finess it in the time given so I decided to move on. 
...
payee_clean_expenses <- read_csv("data/payee_clean.csv")

payee_clean_fb <- payee_clean_expenses |> 
...
**A2.** Ashwani Jain and Peter Franchot were running for governor. Casa in Action is a PAC that aims to "to elect progressive leaders supporting immigrants and communities of color." 

Ashwani Jain ran Facebook ads from April 2021 to July 2022. In this query you can see the ad's estimated audience size, the approximate spend, and an estimated number of impressions. 
...
**A3.** The Democratic primary winners, Wes Moore and his running mate Aruna Miller, are conspicuously missing. They paid a media consultant group called SKDK nearly $6.2 million. 

```{r}

moore_media <- payee_clean |> 
...
**A4.** The committee "Franchot Anderson-Walker for Maryland Slate" appears in the top five of both lists. It appears that the committee for this ticket spent about a quarter of their online advertising funds ($2.4 million total) on Facebook ($5,800ish on this platform). 

```{r}

online_ads <- payee_clean |>
  group_by(committee_name) |> 
  summarise (total = sum(amount)) |>
  arrange(desc(total))
...
**A5** It looks like the big winners in the gubernatorial race did not shell out big bucks for Facebook ads. Or, if they did, they did it through a media consultant group. Moore's successful ticket spent over six million dollars on their consultant firm, SKDK. Maybe the take home there is to hire professionals to manage your media spend instead of chucking money nonstop at Facebook ads (I'm looking at you Jain Ashwani). 
